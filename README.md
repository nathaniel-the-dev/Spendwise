...
| `transaction`          | Expense/income entries                                              | `id`, `amount`, `currency`, `description`, `date`, `type`, `category_id`, `user_id`, `tags`, `notes`, `amount_in_preferred`, `fx_rate`, `fx_rate_at`, `fx_source`                                                                                                          |
...
| `subscription`         | Recurring payments                                                  | `id`, `name`, `amount`, `currency`, `billing_cycle` (`weekly`/`monthly`/`quarterly`/`yearly`/`custom`), `billing_interval`, `category_id`, `start_date`, `next_billing_date`, `end_date`, `status`, `user_id`, `amount_in_preferred`, `fx_rate`, `fx_rate_at`, `fx_source` |
| `subscription_payment` | Payment history for subscriptions                                   | `id`, `subscription_id`, `amount`, `currency`, `paid_date`                                                                                                                                                    |

> Convention: amounts are stored as **positive** numbers; the `type` field (`expense`/`income`) carries meaning. Amounts are totalled in the user's preferred `currency`. A row entered in a foreign currency also carries a frozen `amount_in_preferred` snapshot (`fx_rate`, `fx_rate_at`, `fx_source`) so totals stay plain sums — see [`docs/currency-conversion.md`](docs/currency-conversion.md).
